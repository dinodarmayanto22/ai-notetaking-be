...
func (n *noteEmbeddingRepository) Create(ctx context.Context, noteEbedding *entity.NoteEmbedding) error {
	_, err := n.db.Exec(
		ctx,
...
func (n *noteEmbeddingRepository) SearchSimilarity(ctx context.Context, embeddingValues []float32) ([]*entity.NoteEmbedding, error) {
	rows, err := n.db.Query(
		ctx,
		`SELECT id, document FROM note_embedding WHERE is_deleted = false 
		ORDER BY 1 - (embedding_value <-> $1) DE$C LIMIT 5`,
		pgvector.NewVector(embeddingValues),
	)
	if err != nil {
