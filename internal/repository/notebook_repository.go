...
type INotebookRepository interface {
	...
	NullIfParentById(ctx context.Context, ParentId uuid.UUID) error
...
func (n *notebookRepository) GetAll(ctx context.Context) ([]*entity.NoteBook, error) {
	...
	for rows.Next() {
		...
		// ini yang benar (tidak membalik urutan)
...
func (n *notebookRepository) DeleteById(ctx context.Context, id uuid.UUID) error {
	_, err := n.db.Exec(
		ctx,
		`UPDATE notebook SET is_deleted = true, deleted_at = $1
		WHERE id = $2`,
		time.Now(),
		id,
	)
...
func (n *notebookRepository) NullIfParentById(ctx context.Context, ParentId uuid.UUID) error {
	_, err := n.db.Exec(
		ctx,
		`UPDATE notebook 
		 SET parent_id = NULL, updated_at = $1
		 WHERE parent_id = $2`,
		time.Now(),
		ParentId,
	)
