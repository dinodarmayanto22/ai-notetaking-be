...
func (cs *chatSessionRepository) GetAll(ctx context.Context) ([]*entity.ChatSession, error) {
	rows, err := cs.db.Query(
		ctx,
		`SELECT id, title, created_at, updated_at, deleted_at FROM chat_session WHERE is_deleted = false ORDER BY created_at DESC`,
	)
	if err != nil {
		return nil, err
	}
...
func (cs *chatSessionRepository) GetById(ctx context.Context, id uuid.UUID) (*entity.ChatSession, error) {
	row := cs.db.QueryRow(
		ctx,
		`SELECT id, title, created_at, updated_at, deleted_at FROM chat_session WHERE id = $1 AND = false`,
		id,
	)
